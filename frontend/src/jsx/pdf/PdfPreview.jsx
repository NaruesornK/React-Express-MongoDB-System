...
const PdfPreview = () => {
  ...
  page1.drawText(form1Data.clubName || "", { x: 400, y: 313, size: 15, font, color: rgb(0, 0, 0) });
      ...
      // ✅ ดึงลายเซ็นจาก localStorage
const savedSignature = localStorage.getItem("savedSignature");

if (savedSignature) {
  console.log("✅ พบลายเซ็นใน localStorage!");

  // ✅ แปลง Base64 เป็นไบต์
  const signatureImageBytes = await fetch(savedSignature).then(res => res.arrayBuffer());
  const signatureImage = await pdfDoc.embedPng(signatureImageBytes); // หรือ embedJpg()

  // ✅ แสดงลายเซ็นบนหน้าแรก (กำหนดตำแหน่ง x, y)
  page2.drawImage(signatureImage, {
    x: 130, // ตำแหน่ง X
    y: 650, // ตำแหน่ง Y (ต้องกลับด้าน)
    width: 120, // ขนาดกว้าง
    height: 50,  // ขนาดสูง
  });

  console.log("✅ แทรกลายเซ็นสำเร็จ!");
} else {
  console.warn("⚠️ ไม่พบลายเซ็นใน localStorage!");
}
